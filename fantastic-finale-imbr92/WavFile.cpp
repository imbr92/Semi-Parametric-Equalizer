...
static inline void _swap16Buffer(unsigned short *pData,
                                 unsigned int dwNumWords) {
    unsigned long i;
...
int WavInFile::readHeaderBlock() {
    ...
    if (strcmp(label, fmtStr) == 0) {
        ...
        _swap16((unsigned short &)header.format
                    .byte_per_sample);  // short int byte_per_sample;
        _swap16((unsigned short &)header.format
                    .bits_per_sample);  // short int bits_per_sample;
...
uint WavInFile::getNumChannels() const { return header.format.channel_number; }

uint WavInFile::getNumBits() const { return header.format.bits_per_sample; }

uint WavInFile::getBytesPerSample() const {
    return getNumChannels() * getNumBits() / 8;
}

uint WavInFile::getSampleRate() const { return header.format.sample_rate; }

uint WavInFile::getDataSizeInBytes() const { return header.data.data_len; }
...
void WavOutFile::write(const char *buffer, int numElems) {
    ...
    if (header.format.bits_per_sample != 8) {
        throw runtime_error(
            "Error: WavOutFile::write(const char*, int) accepts only 8bit "
            "samples.");
    }
    assert(sizeof(char) == 1);
